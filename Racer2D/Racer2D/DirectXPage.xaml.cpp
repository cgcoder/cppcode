	...
	m_lastPointValid(false)
{
	...
	m_renderer = ref new DirectXRenderer();
	...
	m_viewController = ref new GameEngine::ViewController();
	m_viewController->Initialize(m_renderer);

	m_viewController->AddView(MENU_VIEW, ref new Racer2D::MenuView());
	m_viewController->CurrentViewName = MENU_VIEW;
...
void DirectXPage::OnRendering(Object^ sender, Object^ args)
{
	if (m_renderNeeded)
	{
		...
		m_viewController->Update(m_timer->Total, m_timer->Delta);

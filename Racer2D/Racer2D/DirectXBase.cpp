...
void DirectXBase::LoadBitmap(LPCWSTR pszImagename, ID2D1Bitmap** ppBitmap)
{
	if (NULL == ppBitmap)
	{
	...
	Microsoft::WRL::ComPtr<IWICBitmapFrameDecode> pWicFrameDecode;
	Microsoft::WRL::ComPtr<IWICFormatConverter> pWicFormatConverter;
	*ppBitmap = NULL;

	DX::ThrowIfFailed(
		m_wicFactory->CreateDecoderFromFilename(pszImagename, 
		nullptr,
		GENERIC_READ,
	...
	DX::ThrowIfFailed(pWicDecoder->GetFrame(0, &pWicFrameDecode));

    DX::ThrowIfFailed(m_wicFactory->CreateFormatConverter(&pWicFormatConverter));
	...
	DX::ThrowIfFailed(m_d2dContext->CreateBitmapFromWicBitmap(pWicFormatConverter.Get(), ppBitmap));
}
...
void DirectXBase::DrawBitmap(ID2D1Bitmap* pBitmap)
{
	m_d2dContext->DrawBitmap(pBitmap);
}

ID2D1DeviceContext* DirectXBase::GetDC()
{
